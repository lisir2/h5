...
// 引入加载动画
var loading = require('../../src/assets/images/loading.gif');
...
export default {
  install( Vue ){
    ...
    Vue.prototype.createElement = function () {
      ...
      str+= '<div class="crop_loading"><div class="crop_content"><img src="'+ loading +'"><div class="crop_text">图片上传中</div></div></div>';
    ...
    /**
     * 外部接口，用于input['file']对象change时的调用  imgname上传图片名称
     * e: 事件源对象 
     * opt: cropper 截取头像的一些参数
     * imgname: 上传图片的名称
     * ischangepicture : 上传图片成功之后，是否修改当前用户头像
     */
    Vue.prototype.clip = function ( e , opt, imgname,ischangepicture ) {
      let self = this;

      imgName = imgname;

      ischangePicture = ischangepicture;
      console.log(ischangePicture)
    ...
    Vue.prototype.postImg = function( imageData ) {
      ...
      console.log(imgName)
      console.log(ischangePicture)
      var file = self.dataURLtoFile(imageData,imgName);  //base64 转为file对象
      var formData = new window.FormData();
      formData.append("file", file);
      $.ajax({
        url: store.state.upLoadImg + '/openapi/common/upload',
        // url: 'http://192.168.0.228:8000/openapi/common/upload',
        type: 'POST',
        ...
        success: function (res) { // 成功回调
          if(res.code == 20000){
            ...
            if(ischangePicture){
              ...
              self.resultObj.src = res.data.url;
