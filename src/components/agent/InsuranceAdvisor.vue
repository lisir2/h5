...
<template>
  <div class="InsuranceAdvisor">
    <van-nav-bar title="保险顾问信息" left-arrow @click-left="onClickLeft"></van-nav-bar>
    ...
    <van-cell-group v-if="userInfo != ''">
      ...
      <van-field label="证件号码" :value="userInfo.agentInfo.identityCode | filterIdentityCode" readonly />
...
export default {
  ...
  computed: {},
  filters: {
    filterIdentityCode: function (value) {
        return value ? value.substr(0,3) + "***********" + value.substr(value.length-4,value.length) : '';
    }
  },
  mounted(){
      var args = this.sign({userId:this.shareMid});
