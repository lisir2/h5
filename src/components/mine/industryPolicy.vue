...
<template>
    <div class='personal'>
        <van-nav-bar title="团险保单" left-arrow @click-left="onClickLeft">
        </van-nav-bar>
        <div class="personalBox">
            <van-tabs v-model="activeName" sticky animated @change="onClick" swipeable  title-active-color="#333" line-width='0'>
                           ...
                           <p></p>
                            ...
                            <div class="coverImg">
                                <div v-for="(insuranceItem,index) in insuranceCompanyInfo" :key="index">
                                    <img v-if="insuranceItem.simpleBrandName == item.goodInsuranceCompanyAlias" :src="insuranceItem.simpleBrandName == item.goodInsuranceCompanyAlias ? baseImagePath + insuranceItem.h5ImgUrl : ''" alt="">
                                </div>
                            ...
                            <p class="fontColor666"><span>订单号：</span><span>{{item.orderNo}}</span></p>
                            <p class="fontColor666"><span>保障时间：</span><span>{{item.startTime}} - {{item.endTime}}</span></p>
                            <p class="fontColor666"><span>被保险人：</span><span>{{item.policyViewInsuredVoList.length != 0 ? item.policyViewInsuredVoList[0].insuredName : ''}}</span><span v-if="item.policyViewInsuredVoList.length > 1" style="margin-left:10px;">等{{item.policyViewInsuredVoList.length}}人</span></p>
                            <p><span>共{{item.policyNum}}份，合计：{{item.policySumPrice}}</span><span><router-link style="color:#008CFF;" :to="{path: 'personalDetail', query: {orderNo:item.orderNo}}">查看详情</router-link></span></p>
                        ...
                        <li v-for="(item,index) in policy_list1" :key="index">
                           <p></p>
                            ...
                            <div class="coverImg">
                                <div v-for="(insuranceItem,index) in insuranceCompanyInfo" :key="index">
                                    <img v-if="insuranceItem.simpleBrandName == item.goodInsuranceCompanyAlias" :src="insuranceItem.simpleBrandName == item.goodInsuranceCompanyAlias ? baseImagePath + insuranceItem.h5ImgUrl : ''" alt="">
                                </div><span >{{item.orderStatus == 1 ? '待支付' : (item.orderStatus == 2 ? '即将生效' : (item.orderStatus == 3 ? '已生效' : (item.orderStatus == 4 ? '已关闭' : (item.orderStatus == 5 ? '即将到期' : (item.orderStatus == 6 ? '已到期' : (item.orderStatus == 7 ? '注销' : (item.orderStatus == 8 ? '退保' : (item.orderStatus == -1 ? '删除' : (item.orderStatus == 0 ? '默认' : '')))))))))}}</span>
                            
                            </div>
                            <p class="goodName">{{item.goodName}}</p>
                            <p class="fontColor666"><span>订单号：</span><span>{{item.orderNo}}</span></p>
                            <p class="fontColor666"><span>保障时间：</span><span>{{item.startTime}} - {{item.endTime}}</span></p>
                            <p class="fontColor666"><span>被保险人：</span><span>{{item.policyViewInsuredVoList.length != 0 ? item.policyViewInsuredVoList[0].insuredName : ''}}</span></p>
                            <p><span>共{{item.policyNum}}份，合计：{{item.policySumPrice}}</span><span><router-link style="color:#008CFF;" :to="{path: 'personalDetail', query: {orderNo:item.orderNo}}">查看详情</router-link></span></p>
                            <p>
                                <router-link :to="{path:'productdetail',query:{productId:item.goodId}}">
                                    <input type="button" value="再次购买" class="againBuy">
        ...
        <!-- 小球 -->
        <div class="globule">
          <router-link to="/historyPolicy/industryHistoryPolicy">
            <div>
              <p>历史</p>
              <p>保单</p>
            </div>
...
export default {
  ...
  mounted() {
    ...
    var that = this;
    // 保单列表
    var args = this.sign({ userId: userId, orderStatus: 0, policyType: 2});
  ...
  methods: {
    ...
    onLoad2() {
      setTimeout(() => {
        for (var i = this.page2 * this.pageSize;i < this.policylist2.length;i++) {
    ...
    onLoad3() {
      setTimeout(() => {
        for (var i = this.page3 * this.pageSize;i < this.policylist3.length;i++) {
    ...
    onLoad4() {
      setTimeout(() => {
        for (var i = this.page4 * this.pageSize;i < this.policylist4.length;i++) {
    ...
    onLoad5() {
      setTimeout(() => {
        for (var i = this.page5 * this.pageSize;i < this.policylist5.length;i++) {
    ...
    showEvalut() {
      this.$router.push({ path: "/myEvaluation" });
    },
    onClick(name, title) {
      var that = this;
      that.finished = true;
