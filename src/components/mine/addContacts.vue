...
<template>
  <div class="addContacts">
    <van-nav-bar title="新增常用联系人" left-arrow @click-left="onClickLeft" />
    <div class="ContactsMessage">
      <ul>
        ...
        <li>
          <span>出生日期</span>
          <input type="text" placeholder="请选择出生日期" id="Birthdate" v-model="Birthdate"/>
        </li>
...
export default {
  ...
  methods: {
    ...
    confirm(val) {
      if (val[2] == undefined) {
        this.$refs.citySelect.value = val[0].name + " " + val[1].name;
      } else {
        this.$refs.citySelect.value = val[0].name + " " + val[1].name + " " + val[2].name;
      }
      this.areaShow = false;
    },
    cancel() {
    ...
    save(){
      ...
      if(this.userName == ''){
        ...
        this.Toast('请请选择出生日期')
      }else if( isPhoneNumber(this.phone) == false){
