...
<template>
  <div class="personal">
    <van-nav-bar title="个险保单" left-arrow @click-left="onClickLeft"></van-nav-bar>
              ...
              <li v-for="(item,index) in policy_list" :key="index">
                ...
                <div class="coverImg">
                  <div v-for="(insuranceItem,index) in insuranceCompanyInfo" :key="index">
                    <img v-if="insuranceItem.simpleBrandName == item.goodInsuranceCompanyAlias"
                ...
                <p class="fontColor666">
                  ...
                  <span v-if="activityCode == 'QD202003240001'" style="float:right;color:#008cff;">{{item.tripType == 0 ? '不延期' : '延期'}}</span>
                </p>
                <p class="fontColor666">
                  <span>保障时间：</span>
...
export default {
  ...
  data() {
    return {
      ...
      phone: this.getQueryString("phone"), //手机号
      args: '', // 签名参数数据
    };
  },
  computed: {},
  mounted() {
    ...
    var that = this;
    // 判断是否是人人巴士渠道进来的
    if(this.activityCode == 'QD202003240001' && this.phone != ''){
      this.args = {
    ...
    // 保单列表
    // this.$axios.post('/brg/openapi/h5/policyDetailView/policyList',this.args).then(res => {
    api.policyList(this.sign(this.args)).then(res => {
      var response = res;
  ...
  methods: {
    ...
    async listMessage(status) {
      ...
      var Data = "";
      // 修改状态
      that.args.orderStatus = status;
      // await this.$axios.post('/brg/openapi/h5/policyDetailView/policyList',that.args).then(res => {
      await api.policyList(that.sign(that.args)).then(res => {
        var response = res;
        if(response.code == 20000){
          var responseData = response.data;
          for (var i = 0; i < response.data.length; i++) {
