          ...
          >
            <ul class="policylist">
              <li v-for="(item,index) in policy_list" :key="index">
                ...
                <p class="fontColor666">
                  <span>保障时间：</span>
                    <span>{{item.startTime}} 00:00:00 - {{item.endTime}} 23:59:59</span>
                </p>
                <p class="fontColor666">
                  <span>被保险人：</span>
                  <span v-if="item.count==1">{{item.insuredName}}</span>
                  <span v-else-if="item.count>1">{{item.insuredName}}等{{item.count}}人</span>
                  <span v-else></span>
                </p>
                <p>
                  <span>保单金额：{{item.totalFee}}元</span>
                  <span>
                    <!-- <router-link
                      style="color:#008CFF;"
...
export default {
  ...
  methods: {
    seeDetails(item) {
      var that = this;
      if (item.productName == "环晟-平安家政服务综合保险") {
    ...
    async listMessage(status, page) {
      ...
      let total;
      await this.$axios
        .post("/zbjj/Phone/queryHistoricalPolicy", args) //zbjj/Phone/queryHistoricalPolicy
        .then(res => {
          console.log(res);
          total = res.data.industryPolicyTotal;
          let response = res.data;
          let responseData = [];
          if (response.success) {
            Data = response.industryPolicyList;
          } else {
